...
try {
    ...
    Invoke-RestMethod -Uri 'http://127.0.0.1:8765/api/control' -Method Post -Headers @{ 'X-CSRF-Token' = $wxState.csrf } -ContentType 'application/json' -Body '{"enabled":false}' | Out-Null
